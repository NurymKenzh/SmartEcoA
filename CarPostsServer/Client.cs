    ...
    class Client
    {
        ...
        private void ParseData(string jsonString)
        {
            try
            {
                ...
                if (clientJsonData.CarPostId != null)
                {
                    Console.WriteLine($"{DateTime.Now} >> Get data from CarPost {clientJsonData.CarPostId}{Environment.NewLine}");

                    var carPostId = Convert.ToInt32(clientJsonData.CarPostId);
